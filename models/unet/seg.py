...
class Segmentation(CAM):
    ...
    def generate_pseudo_label(self, imgs, cls_label_true, original_image_size, fg_thres=0.5, *args, **kwargs):
        with torch.set_grad_enabled(False):
            cams = [self.forward_cam(img[0]) for img in imgs]
            strided_cam = [F.interpolate(torch.unsqueeze(o, 1), original_image_size,
                                         mode='bilinear', align_corners=False) for o in cams]
            strided_cam = torch.sum(torch.stack(strided_cam, 0), 0)[:, 0, :original_image_size[0],
            ...
            fg_conf_cam = np.pad(cams, ((1, 0), (0, 0), (0, 0)), mode='constant', constant_values=fg_thres)
